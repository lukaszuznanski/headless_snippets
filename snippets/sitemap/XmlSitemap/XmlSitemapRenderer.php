...
namespace Vendor\Package\XmlSitemap;
...
class XmlSitemapRenderer
{
    ...
    protected function defineUrl(array $data): string
    {
        ...
        $typoLinkConfig = [
            'parameter' => 1,
            'additionalParams' => '&sitemap=' . $data['key'] . '&type=' . $GLOBALS['TSFE']->type,
            'forceAbsoluteUrl' => 0,
            'useCacheHash' => $this->config['url']['useCacheHash'] ?? 0
        ];

        $test = $cObj->typoLink_URL($typoLinkConfig);
        if (isset($this->configuration['settings']['domain'])) {
            $url = $this->configuration['settings']['domain'] . $cObj->typoLink_URL($typoLinkConfig);
        } else {
            $url = $cObj->typoLink_URL($typoLinkConfig);
        }
